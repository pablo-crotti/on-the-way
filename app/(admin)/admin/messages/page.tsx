    ...
    <div className="w-full h-full flex-col justify-center align-center p-4">
      ...
      <div className="my-4 flex flex-row justify-between items-center">
        ...
        <button onClick={deleteMessages} className="mr-6">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="flex-shrink-0 w-5 h-5 text-darkbg-500 transition duration-75 dark:text-darkbg-400 group-hover:text-darkbg-900 dark:group-hover:text-white">
          ...
          <tbody>
            {messages.map(
              (message: {
                ...
                <tr className="bg-white border-b dark:bg-darkbg-800 dark:border-darkbg-700 hover:bg-darkbg-50 dark:hover:bg-darkbg-600" key={message.id}>
                  <th className="px-6 py-4 whitespace-nowrap">
                    <input type="checkbox" id={message.id} onChange={(event) => handleCheckbox(event)} className="w-4 h-4 text-primary bg-darkbg-100 border-darkbg-300 rounded focus:ring-primary dark:focus:ring-primary dark:ring-offset-darkbg-800 focus:ring-2 dark:bg-darkbg-700 dark:border-darkbg-600" />
                  </th>
                  <th
                    scope="row"
      ...
      <div className="flex flex-col items-center">
        <span className="text-sm text-darkbg-700 dark:text-darkbg-400">
          Page <span className="font-semibold text-darkbg-900 dark:text-white">{currentPage}</span> sur <span className="font-semibold text-darkbg-900 dark:text-white">{totalPages}</span> pour <span className="font-semibold text-darkbg-900 dark:text-white">{totalMessages}</span> messages
        </span>
        <div className="inline-flex mt-2 xs:mt-0">
          <a href={"/admin/messages?page=" + (currentPage - 1 < 1 ? 1 : currentPage - 1)} className="flex items-center justify-center px-4 h-10 text-base font-medium text-white bg-darkbg-800 rounded-s hover:bg-darkbg-900 dark:bg-darkbg-800 dark:border-darkbg-700 dark:text-darkbg-400 dark:hover:bg-darkbg-700 dark:hover:text-white">
            Précédent
          </a>
          <a href={"/admin/messages?page=" + (currentPage + 1 > totalPages ? totalPages : currentPage + 1)} className="flex items-center justify-center px-4 h-10 text-base font-medium text-white bg-darkbg-800 border-0 border-s border-darkbg-700 rounded-e hover:bg-darkbg-900 dark:bg-darkbg-800 dark:border-darkbg-700 dark:text-darkbg-400 dark:hover:bg-darkbg-700 dark:hover:text-white">
            Suivant
          </a>
        </div>
      </div>
