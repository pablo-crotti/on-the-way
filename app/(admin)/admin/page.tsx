...
export default function Home() {
  ...
  const getTotalPageViews = () => {
    ...
    fetch(
      `/api/analytics?category=os_name&from=${encodeURIComponent(
  ...
  useEffect(() => {
    ...
    fetchPodcasts().then((data) => {
      const dataSorted = data.sort(
        (a: any, b: any) => b.publish_time - a.publish_time
      );
      setTotalEpisodes(dataSorted.length);

      if (dataSorted.length == 1) {
        setLastPodcasts([dataSorted[0]]);
        ...
        <div className="flex justify-center flex-wrap md:justify-center gap-4">
          ...
          <div className="flex flex-row items-start gap-4 p-4 w-2/5 bg-white border border-gray-200 rounded-lg shadow md:flex-col  dark:border-darkbg-700 dark:bg-darkbg-800">
            ...
            {lastPodcasts.map((podcast: any, index: number) => (
              <div
                key={index}
                className="flex items-center w-full bg-white border border-gray-200 rounded-lg shadow flex-row md:max-w-xl dark:border-darkbg-700 dark:bg-darkbg-800"
              >
                <img
                  className="object-cover h-full  md:min-w-28 md:max-w-28 aspect-square rounded-none rounded-s-lg"
                  src={podcast.logo}
                  alt=""
                />
                <div className="flex flex-col justify-between p-4 leading-normal w-2/3">
                  <h4 className="w-full break-words text-base text-left font-medium tracking-tight text-gray-900 dark:text-white">
                    Série {podcast.season_number}
                  </h4>
                  <h5 className="w-full break-words text-lg text-left font-bold tracking-tight text-gray-900 dark:text-white">
                    Episode {podcast.episode_number}
                  </h5>
                  <p className="text-gray-500 dark:text-gray-200">
                    Publié le :{" "}
                    {new Date(podcast.publish_time * 1000).toLocaleString(
                      "fr-CH",
                  ...
                  <p className="text-gray-500 dark:text-gray-200">
                    Statut :{" "}
                    {podcast.status === "publish" ? "Public" : "Brouillon"}
                  </p>
                  <div className="flex flex-row gap-2 mt-2">
                    <a href={`/admin/episodes/${podcast.id}`}>
                      <svg
                        ...
                        viewBox="0 0 24 24"
                        strokeWidth="1.5"
                        className="w-6 h-6 text-green-700 fill-none stroke-current"
                      >
                        <path
                          strokeLinecap="round"
